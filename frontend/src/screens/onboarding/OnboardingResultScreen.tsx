...
import { OnboardingStackParamList, RoutineBlock, ParentGoalData } from '../../types';
import { colors, spacing, fontSizes, radius, shadow } from '../../theme';

type Props = {
  navigation: NativeStackNavigationProp<OnboardingStackParamList, 'OnboardingResult'>;
  route: RouteProp<OnboardingStackParamList, 'OnboardingResult'>;
};
...
const GoalCard: React.FC<{ goal: ParentGoalData; index: number }> = ({ goal, index }) => {
  const { t } = useTranslation();
  const [expanded, setExpanded] = useState(false);
  const colors2 = ['#5B8DEF', '#F7A44A', '#4CAF82'];
  const color = colors2[index] ?? colors.primary;
      ...
      {expanded && (
        <View style={styles.goalExpanded}>
          <View style={styles.goalExpandedBlock}>
            <Text style={styles.goalExpandedLabel}>{t('onboarding.goal_why')}</Text>
            <Text style={styles.goalExpandedText}>{t(`goal.${goal.goal_key}.why`)}</Text>
          </View>
          <View style={styles.goalExpandedBlock}>
            <Text style={styles.goalExpandedLabel}>{t('onboarding.goal_how')}</Text>
          ...
          <View style={styles.milestoneRow}>
            {(['week1','week2','week4'] as const).map(w => (
              <View key={w} style={styles.milestone}>
                <Text style={[styles.milestoneWeek, { color }]}>{t(`onboarding.goal_label_${w}`)}</Text>
...
export const OnboardingResultScreen: React.FC<Props> = ({ navigation, route }) => {
  const { t } = useTranslation();
  const insets = useSafeAreaInsets();
  const { routine, goals, childName } = route.params;
  const { setUser } = useAuthStore();
  const { clearAll } = useChildStore();
  ...
  const finish = () => {
    // Mark onboarding done in local store so RootNavigator reroutes
    setUser({ onboarding_completed: true } as any);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + spacing.xl }]}>
        <Text style={styles.headerTitle}>{t('onboarding.result_title')}</Text>
        <Text style={styles.headerSubtitle}>
          {t('onboarding.result_subtitle', { name: childName })}
        </Text>
      </View>
      ...
      <View style={styles.tabs}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'routine' && styles.tabTextActive]}>
            {t('onboarding.result_routine_tab')}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
                ...
                <View key={cat} style={styles.routineSection}>
                  <View style={[styles.routineSectionHeader, { backgroundColor: CATEGORY_COLORS[cat] + '20' }]}>
                    <Text style={[styles.routineSectionTitle, { color: CATEGORY_COLORS[cat] }]}>
                      {t(`onboarding.routine_section_${cat}`)}
                    </Text>
                  </View>
      ...
      <View style={styles.footerBar}>
        <TouchableOpacity style={styles.doneBtn} onPress={finish} activeOpacity={0.85}>
          <Text style={styles.doneBtnText}>{t('onboarding.result_done')}</Text>
...
const styles = StyleSheet.create({
  ...
  header: {
    backgroundColor: colors.primary, paddingTop: 0,
    paddingBottom: spacing.xl, paddingHorizontal: spacing.lg,
  },
  headerTitle: { fontSize: fontSizes['2xl'], fontWeight: '900', color: '#fff', marginBottom: 4 },
  headerSubtitle: { fontSize: fontSizes.md, color: '#fff', opacity: 0.9, lineHeight: 22 },
  ...
  routineSectionHeader: { borderRadius: radius.md, paddingVertical: 6, paddingHorizontal: spacing.md, marginBottom: spacing.sm },
  ...
  footerBar: {
    ...
    padding: spacing.lg, paddingBottom: 34, backgroundColor: colors.background,
  ...
  doneBtnText: { fontSize: fontSizes.lg, fontWeight: '800', color: '#fff' },
});
